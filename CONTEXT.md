...
- **Animation handoff blend**: The fixed-duration transition used at the end of any interactive motion event, blending from the event's final root/pose to the live tracked root/pose regardless of how far the live anchor has moved during the event. For Human this blends both the root transform and every humanoid bone's local rotation; blending only the root left the body pose snapping instantly into the tracked pose while only the root caught up smoothly. Animal handoff still only blends the root (known v1 gap).
