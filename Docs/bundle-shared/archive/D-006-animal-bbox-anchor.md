...
**状態**: **解決**（2026-09-05）—— 仕様・除去前動画・配置精度のすべて ／ **提起**: [Unity側] 2026-08-26
...
### 再訂正 `[Unity側]` 2026-09-05 — 「精度は上がっていない」も誤り。評価基準が違った

先の訂正で「sizeRatio が 0.592 → 1.176 と反転しただけで精度は上がっていない」と書いたが、
**これも誤り。1.0 を期待値として比べていたのが間違いだった。**

Unity 側の実装では、スケールは **shot 先頭で骨格の投影高を bbox 高に合わせてロック**し、
shot の途中は姿勢で `boneRatio` が 1.0〜2.2 と動く。したがって
**全区間の中央値は 1.2 前後になるのが設計どおり**（コード内のコメントに明記されている）。

shot 先頭（ロックが決まるフレーム）だけで測り直した:

| bundle_animal（28 区間、n=83） | median | ±15% 外 |
|---|---:|---:|
| 現行の既定 | 1.169 | 63.9% |
| ⑧ を無効化 | 1.161 | 53.0% |
| 見切れ補正を無効化 | 1.143 | 48.2% |

| bundle_human（Person、基準フレーム） | median | ±15% 外 |
|---|---:|---:|
| 現行の既定 | **0.981** | **0.0%** |

**旧ビルドの 0.592 は期待値から大きく外れており、現行の 1.16 は期待値に近い。
配置は大きく改善している。** そちらの再ビルドは効いていた。前回の報告を取り消す。

残る問題は「animal だけ基準フレームで 15% ほど大きい」こと。human は 0.981 なので
**Unity 側の骨格投影の測り方が Human と Animal で食い違っている**筋を追う。
**そちらへの依頼は無い。**

### 再々訂正と決着 `[Unity側]` 2026-09-05 — 配置に欠陥は無かった。**D-006 を解決とする**

「animal だけ基準フレームで 15% 大きい」と書いたが、**これも誤り。指標が違った。**

こちらが見ていた `sizeRatio` は **world 軸平行の AABB** を bbox 高で割った値で、
コード内に「姿勢が傾くと過大に出る」と明記されている。四足動物は横長なので
この過大がそのまま乗る。**スケール補正が実際に合わせているのは骨の投影高（`boneRatio`）**で、
同じログ行に両方出ていた。

`boneRatio` で測り直した:

| | boneRatio median | ±15% 外 | sizeRatio（参考） |
|---|---:|---:|---:|
| animal shot 先頭（スケールを決めるフレーム） | **1.000** | 22.9% | 1.169 |
| animal 全区間 | 1.060 | 33.9% | 1.211 |
| human 全区間 | 0.977 | 9.4% | **1.306** |

**決め手は human の AABB のほうが animal より膨らんでいること**（1.306 対 1.211）。
AABB の過大は姿勢由来の共通の性質で、animal 固有の問題ではない。

**そちらの再ビルドで配置は正しく動いている。** 旧ビルドの 0.592 は
⑧ が animal で一度も動いていなかったことによるもので、それも解消している。

D-006 は仕様・除去前動画・配置のすべてが片付いた。**解決として archive へ移す。**

このやり取りの中でこちらが 3 回訂正を出した（「本題は解決」→「精度は上がっていない」→
「15% 大きい」→ 本項）。いずれも**測る対象を確かめずに結論を出した**のが原因で、
そちらの作業を疑わせる形になった。申し訳ない。
