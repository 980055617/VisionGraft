...
### しっぽが動かない理由は入力ではなくコードの減衰だった（2026-08-28）

実機で「犬・猫とも動画の中よりしっぽが動かない」と指摘され、原因を特定した。

```csharp
// Provisional damping for tailBase/tailMid body_pose
// - starting conservative at half strength until visually verified.
private const float TailBodyPoseScale = 0.5f;
```

2026-07-16 に「一部のモデルでしっぽが体にめり込む」という報告を受けて入れた暫定の減衰。
コメントに **「実機で見て確認したら調整・撤去せよ」** と書かれていた。**その確認が今回できた
ので等倍（1.0）に戻した。**

#### 入力側にはしっぽの動きがある

`meta.bin` の body_pose の絶対量（恒等回転からの角度、犬 track0）:

| joint | median | p90 | max |
|---|---|---|---|
| 15 Neck | 29.3° | 42.4° | 63.2° |
| **25 TailBase** | **28.6°** | **42.5°** | **50.1°** |
| 7 / 11 前肢 Upper | 9.5 / 12.4° | 18.5 / 24.8° | 41.2 / 47.4° |
| 17 / 21 後肢 Upper | 18.9 / 13.0° | 31.6 / 26.9° | 45.6 / 37.8° |
...
| | しっぽ | 脚 |
|---|---|---|
| 入力の大きさ | **28.6°** | 9.5〜18.9° |
| コードによる減衰 | **0.5 倍**（撤去した） | なし |

**しっぽは「入力はあるのに半分にしていた」、脚は「そもそも入力が小さい」。**
しっぽは改善が見込めるが、脚は入力側の問題なので Unity 側では動かせない。

#### 残る制限: tailTip は駆動されない

`ShouldKeepBindPoseForJoint(27..31)` により **joint 27（tailTip）は body_pose を受け取らない**。
SMAL 側の子（Tail4）に対応する Unity ボーンが無いため。しっぽは根元と中間だけが動き、
先端は親追従になる。今回は触っていない。

#### 再発しうる副作用

等倍に戻すと当初の懸念（**一部モデルでしっぽが体にめり込む**）が再発しうる。実機で見て
問題があれば 0.75 など中間値へ。定数 1 つ。

### 「1 秒あたりの上半身が元動画と違う」の正体（2026-09-10）— 回転は合っている。体格が違う

3 面比較の動画版（`Docs/log-analysis/three_panel_video.py`）で
「上半身の体勢が元動画と全然違う」と繰り返し指摘されている区間を見られるようにしたので、
**その差が回転の追従不足なのか体格差なのかを分けて測った。**

全編 2154 フレームの `[KP2D]`（`bundle_human.svb` = `..._maskanchor.svb`、
実機に合わせた `-targetFps 72`）を使う。**指標を 2 つに分けるのが要点。**

| 指標 | 中央値 | p10 | p90 | 何を見ているか |
|---|---:|---:|---:|---|
| **肩の線の傾きの差**（mdl − src）| **+0.64°** | −2.87° | +3.06° | **体格に依らない。**回転の追従 |
| **肩幅/腰幅 の mdl ÷ src** | **0.465 倍** | 0.425 | 0.497 | 体格。1.0 なら同じ体つき |
| 胴の長さ mdl ÷ src | 1.266 倍 | 1.065 | 2.464 | 体格 |

**回転は追えている。**肩の線の傾きは中央 0.6°、8 割が ±3° に収まる。
肩幅/腰幅 の元データとの相関も 0.882 あり、ひねりの動きそのものは届いている。

**外れているのは体格。**肩幅/腰幅 の比は **0.465 倍**で、しかも
**p10 0.425 〜 p90 0.497 と分布が極端に狭い。**追従が崩れているならフレームごとに
散らばるはずで、**ほぼ一定の係数**という形は「毎フレーム同じだけずれている」＝体格差を意味する。

つまり **モデルは人の 2 倍以上「なで肩で腰が広い」**。胴は 27% 長い。
2026-09-08 に記録した「hip width 1.76×・shoulder 0.88×」（積で 0.50）と整合する。
**同じ結論に別経路で到達した。**

#### これが意味すること

**姿勢適用のコードを直しても、この見え方は変わらない。** FK は回転しか運ばないので、
肩幅・腰幅・胴の長さは**モデル側の骨の位置**で決まる。回転は既に合っている。

打ち手は姿勢側ではなくモデル側になる:

| 案 | |
|---|---|
| 体型の近いモデルに差し替える | 最も素直。ただし見た目の好みはユーザーが決める |
| ボーンのローカル位置を SMPL の betas に合わせて調整する | Humanoid Rig の骨位置を触ることになる。影響範囲が広い |
| 現状を受け入れる | 「モデルは別人」という前提なら破綻ではない |

**どれを採るかは絵を見てユーザーが決めること。**こちらから revert も採用もしない。

#### 外れ値（別件、未調査）

肩の線の傾きが大きく食い違うフレームが少数ある: f2063（+172°）、f1084-1086（約 −119°）、
f2110-2111（約 −100°）。**左右反転か、被写体が寝ている区間**と思われる。
中央値の話とは別機序なので、追うなら分けて扱う。

### Animal の棚卸し（2026-09-10）— 「27〜30 秒で頭が動かない」の実測

ユーザー報告:「animal の 27〜30 秒、実際の動物は頭が動いているのにモデルは止まっている」。
「しっぽと頭はそんなに動かないのでは」という問いも併せて検証した。

測定は `bundle_animal.svb`（推奨 `b9a9d5fe…`）、`-targetFps 72 -diagLogs true -diagEveryN 1`。
`meta.bin` の SMAL block を直接読む移植を新規に作った（`smal_read.py`、
`StreamingStereoVideoPlayer.AnimalSmal.partial.cs` の `StoreSmalBlockFromBin` を写したもの）。

**読み取りで 1 度間違えた。**`flags` のビットは **`0x1`=skeleton / `0x2`=SMPL / `0x4`=SMAL**。
`train_depth_order.py` の SMPL 用（`0x2`）をそのまま使って「SMAL block が 0 件」という
誤った結論を出しかけた。**animal は `0x4`。**

#### 1. どの関節が実際に駆動されているか — 34 中 12

`SmalRestDirByJoint` に幾何補正が登録されている関節だけが `body_pose` で駆動される。
それ以外は `tw[joint] = parentTW * bindLoc`＝**親に付いて動くだけ**。

| | 関節 |
|---|---|
| **駆動** | 前脚 7,8 / 11,12・首 15・**頭 16**・後脚 17,18 / 21,22・**しっぽ 25,26** |
| **親追従のみ** | 肉球 9,13,19,23・10,14,20,24・**しっぽ先 27〜31**・口 32・**耳 33,34** |

**CLAUDE.md の「head は body_pose 未適用」は古い。**頭は 2026-09-06 に登録済みで、
実行ログの `REST-CHECK` にも `joint=16` が 97 回出る（＝駆動側の分岐に入っている）。
**「頭が駆動されていないから動かない」ではない。**

#### 2. データには動きがある

`meta.bin` の `body_pose` を直接測った（track 0）。

| 関節 | rest からのずれ（全編中央）| **27〜30 秒の総回転** |
|---|---:|---:|
| 首 15 | 29.3° | **48.0°** |
| **頭 16** | 20.5° | **39.2°** |
| しっぽ 25 | 28.6° | 36.6° |
...
**しっぽも頭も、データ上はよく動いている。**しっぽ先（27）は 34.4° 動いているのに
**親追従のみなので、その動きは捨てられている。**

#### 3. 27〜30 秒で頭は「止まっている」のではなく「向きが 125° ずれている」

`[ANIMALKP]`（ボーンの現在の向きと keypoint が示す向きの角度差）を区間別に集計:

| ボーン | 全編の中央値 | **27〜30 秒の中央値** | 最大 |
|---|---:|---:|---:|
| **Head** | 35.0° | **125.0°** | 146.0° |
| LRUp | 51.0° | 79.0° | 88.0° |
| LFPaw | 31.0° | 71.0° | 86.0° |
| LFLo | 28.0° | 67.0° | 89.0° |

**ユーザーが指した区間で、頭の誤差が 35° → 125° に跳ねている。**
ほぼ真逆を向いている状態で固まるので、「動いていない」と知覚される。
**駆動はされているが、結果が合っていない。**

#### 4. 絵で見るともっと大きな不一致がある

`Docs/tmp/animal_head_27-30s.png`（元動画とモデルを 27〜30 秒で並べたもの）。

**元動画の犬は伏せてカメラを向いているのに、モデルは 4 本足で立っている。**
頭の向き以前に胴と脚の姿勢が合っていない。
`[ANIMALKP]` の後肢 Upper が全編中央 51〜52° というのがこれに対応する。

**採否・破綻の判断はユーザーが絵を見て行う。**ここでは事実だけ記録する。

#### 5. rest 方向のずれ

`REST-CHECK` の `restDirAngleDeg`（SMAL の rest 方向と Unity ボーンの rest 方向の角度差）:

```
joint=7 101.3  joint=8 90.4   joint=11 112.2  joint=12 108.9
joint=15 103.7 joint=16 110.3 joint=17 112.3  joint=18 117.9
joint=21 102.5 joint=22 125.7 joint=25 130.7  joint=26 128.4
```

コード内のコメントは「150° 以上なら `FromToRotation` の軸が不定」と書いているので
その閾値は超えていないが、**全関節が 90〜131° ずれている**。
既存の記述「70〜80° のずれ自体が疑いの対象」に照らすと、これも追う価値がある。

#### 6. 誤報を 1 件つぶした（track 1 は表示されていないのではない）

`[PLACE]` に `track=1` が一度も出ないので「2 頭目が表示されていない」と書きかけたが、
**track 1 は f1146-2119（38.2〜70.6 秒）にしか存在せず、この実行は `-playSeconds 40` で
届いていなかっただけ**だった。track 1 の検証は別途、長い実行で行う。

#### 未検証（次にやること）

- **track 1（38.2〜70.6 秒）を一度も測っていない。**長い実行が要る
- しっぽ先（27〜31）・耳（33,34）・口（32）を駆動していない件の影響量
- `body_pose` の平滑化 half-life 0.12 秒の根拠が古い可能性
  （コメントは「生の body_pose は 20〜100° の跳ね」と書くが、
  現行 bundle の実測はフレーム間中央 0.4〜0.6°・p90 5〜7°）

### 根本原因（2026-09-10）— Animal の FK は親の姿勢を積んでいない

測定 B（`-noBend true`＝曲げを恒等に固定し「bind pose を globalOrient で回しただけ」にする）を
27〜30 秒で実行し、通常再生と並べた（`Docs/tmp/animal_nobend_ab.png`）。

**絵がほとんど変わらない。**数値でも:

| | body_pose あり | 切った | 差 |
|---|---:|---:|---:|
| `[ANIMALKP]` 全 15 ボーンの中央値の差 | — | — | **中央 −4.0°／平均 −2.3°** |

**`body_pose` を丸ごと捨てても誤差が変わらない。むしろ僅かに良くなる。**
つまり **`body_pose` は見た目に効いていない。**モデルは実質「bind pose を globalOrient で
回しただけ」の状態で、犬が伏せていても立ったままになる。

#### 機構

`AnimalSmalFkApplier` の駆動分岐は

```csharp
Quaternion restWorldRot = worldFk0 * boneBindWorld;   // ← bind pose を globalOrient で回しただけ
...
tw[joint] = bendUnity * restWorldRot;                  // ← 親の姿勢が入っていない
```

**`restWorldRot` は「bind pose の世界回転」で、親がその時点で取っている姿勢を含まない。**
`parentTW`（＝この walk で既に確定している親の現在の世界回転）は、
この分岐では**一度も使われていない**。使っているのは

- `bone == null`（仮想 spine 1-6）… `tw = parentTW * smalLocal`
- `ShouldKeepBindPoseForJoint`（しっぽ先・肉球など）… `tw = parentTW * bindLoc`

の 2 つだけ。**つまり「駆動される関節ほど親を無視し、駆動されない関節だけが親に従う」**
という逆転が起きている。

Unity のヒエラルキーでは親を回せば子も動くが、**その直後に子の世界回転を
`bendUnity * restWorldRot` で上書きするので、親の曲げは向きとしては消える**
（位置だけは親に付いていく）。

結果として各ボーンは「bind pose から自分のローカル曲げぶん（20〜30°）だけずれた向き」に
留まる。**伏せる・座るのように胴と四肢が合成されて初めて成立する姿勢は、原理的に作れない。**

これは 27〜30 秒で頭が動かなく見える件とも整合する: **頭（16）は首（15）の 48° の振りを
向きとして受け取らない。**

#### この形になった経緯

コード内のコメントは `parentTW` を使わない理由を
「親が仮想 SMAL 関節（例 6）のことがあり、そのボーンの実際の Unity 親と対応しないため」
と説明している（ADR-0002）。**理由自体は実在する問題を指しているが、
その対処として親の姿勢を丸ごと捨てている。**

#### 直し方（案・未採用）

各関節の「rest からのずれ」を親から積む:

```
dev[0] = identity
駆動される関節   : dev[joint] = dev[parent] * bendUnity ; tw[joint] = dev[joint] * restWorldRot
それ以外の関節   : dev[joint] = dev[parent]
```

`restWorldRot` はそのまま使うので「仮想親とボーン親の食い違い」問題を再導入しない。
**採否は絵を見てユーザーが決める。**フラグで A/B できるようにする。

#### 併せて直した欠陥（別件・小さい）

`smalPosedDir` が `rawLocal`（平滑化前）を使っていた。上で計算・保存している
`smalLocal` は **実ボーンには一度も使われていなかった**（`git log -S` で確認、実装当初から）。
`smalLocal` に変更済み。ジッタの中央値は下がる（頭 10.8° → 7.8°）が、
**見た目の支配要因ではない**（`Docs/tmp/animal_smoothing_ab.png`）。

#### 注意: `[ANIMALKP]` の前肢・末端の対応づけは裏が取れていない

2026-08-28 のトポロジー復元と突き合わせると、`LFUp=(12,8)` / `RFUp=(13,9)` は
復元された木（12–9、13–8）と**左右が交差している**。`LFPaw=(14,3)` / `RFPaw=(15,4)` も
木では 3 が 9 に、4 が 13 につながっており一致しない。
**後肢の近位（7→10、7→11）と Lo（8→14、9→15）は木と一致**するので、
本節の結論はそちら側の数字と測定 B（body_pose の有無）に依っている。

### 訂正と真因（2026-09-10）— データ自体が「立ち」で、伏せを表していない

前節で「Animal の FK は親の姿勢を積んでいない」を真因として書いたが、
**それは真因ではなかった。訂正する。**

親の曲げを積む経路を実装して A/B を取った（`accumulateSmalParentBend`、既定 OFF、
`-accumBend true`）。結果:

| | 従来 | 親の曲げを積む |
|---|---:|---:|
| `[ANIMALKP]` 全編中央値の差 | — | **中央 +0.0°／平均 +1.4°**（Head は +12°） |
| 見た目（`Docs/tmp/animal_accum_ab.png`）| 立っている | **ほぼ変わらず立っている** |

**犬は伏せたままにならない。**そこでデータ側を直接調べた。

#### 測定 1: `body_pose` を SMAL の rest skeleton に当てて形を組む

`Docs/smal-rest-skeleton.json`（+X 頭方向 / +Y 左 / +Z 上、足が負 Z）に
`body_pose` だけを当てて FK し（**`global_orient` は外す**＝体の向きではなく形を見る）、
**体幹（spine2）から足先 4 点までの上下の落差**を測った。
立っていれば rest と同じ、伏せれば大きく縮む。

| 区間 | 落差 | rest 比 |
|---|---:|---:|
| rest（立ち姿勢） | 0.5227 | 1.00 |
| 全編 | 0.4966 | 0.95 |
| **27〜30 秒（元動画では伏せている）** | **0.4201** | **0.80** |
| f901-1145 | 0.5145 | 0.98 |

**伏せている区間でも rest の 0.80 倍。**本当に伏せていれば 0.2〜0.3 倍になるはず。

#### 測定 2: keypoints3d による独立確認（rest skeleton に依存しない）

体軸（kp7 尾の付け根 → kp18 頭）からの足先 4 点（kp3/4/5/6）の垂線距離を、
体長で割って測った。**回転に依らない量**なので向きの影響を受けない。

| 区間 | 足の体軸からの距離 ÷ 体長 |
|---|---:|
| 全編 | 0.699 |
| **27〜30 秒** | **0.675（全編の 0.97 倍）** |
| f901-1145 | 0.670 |
...
**`body_pose` と `keypoints3d` の両方が、伏せを表していない。**
2 つは独立な経路（一方は rest skeleton 経由の FK、もう一方は 3D 点の幾何）で、
どちらも同じことを言っている。

**つまり利用側の FK をどう直しても、この見え方は直らない。**
AniMer の当てはめが**姿勢の振れ幅を大幅に過小評価している**、というのが真因。
`[ANIMALKP]` の後肢 Upper 51〜52°、測定 B で `body_pose` を切っても誤差が変わらないこと、
どちらもこれで説明がつく（**動いていない信号を当てているので、当てても当てなくても同じ**）。
...
| | 状態 |
|---|---|
| `smalPosedDir` を `smalLocal` に（平滑化の配線） | **採用**。実装当初からの欠陥。ジッタの中央値は下がる（頭 10.8° → 7.8°） |
| `accumulateSmalParentBend`（親の曲げを積む） | **既定 OFF のまま保留**。コードの構造としては実在する欠陥だが、**データが律速と判明したので今直しても効果を測れない**。指標は微悪化（平均 +1.4°）、見た目は不変。データ側が改善したら再評価する |

### 【重要な訂正】（2026-09-10 続き）— データは正しい。落ちているのは利用側

直前の節「データ自体が『立ち』で、伏せを表していない」は **誤り。撤回する。**

#### なぜ間違えたか

「立ち／伏せ」を 2 つの指標で測った:

- A: `body_pose` を rest skeleton に当てた FK の「体幹→足先の落差」÷ rest
- B: `keypoints3d` の「足先の体軸からの距離」÷ 体長

**どちらの指標も姿勢の違いを拾えていなかった。**座っている区間（17.8〜21.6 秒）で測ると

| 区間 | A | B |
|---|---:|---:|
| 全編 | 0.95 倍 | 0.705 |
| **17.8〜21.6 秒（元動画では座っている）** | **0.95 倍** | **0.725** |
| 27〜30 秒（伏せ） | 0.80 倍 | 0.677 |

**座っている区間が全編と同じ値**になっている。座位では後肢は畳まれるが前肢は伸びたままで、
体軸自体も起き上がるため、**この 2 つの量では座位と立位が区別できない。**
指標が鈍いだけなのに「データが立っている」と読んだ。

**数値だけで結論に行ったのが原因。**絵を先に見るべきだった。

#### 絵で見ると逆だった

`Docs/tmp/animal_4panel_full.png`（f573/606/639、全身が画面内に収まる区間を選んだ）。
**元動画 / keypoints3d / `body_pose` から組んだ骨格 / モデル配置** の 4 枚。

| | 何が写っているか |
|---|---|
| 元動画 | 犬が**座って**いる（左向き、前肢は伸び、後肢は畳まれている）|
| keypoints3d（緑）| **座位を正しく捉えている。**犬に重なる |
| **`body_pose` から組んだ骨格（黄）** | **こちらも座位。**頭が上がり、胴が起き、前肢が下に伸びている |
| モデル配置 | **4 本足で立っている。**まったく別の姿勢 |

**`body_pose` は座位を持っている。**標準の運動連鎖
（`R[j] = R[parent] * local[j]`、`smal_fk_points` に実装）で組み直すと、
**元動画と同じ座位が出る。**

**つまりデータは正しく、利用側の FK が落としている。**

#### 直前の節との関係

「Animal の FK は親の姿勢を積んでいない」（`tw[joint] = bendUnity * (worldFk0 * boneBindWorld)`）
という**構造の指摘は正しかった。**撤回するのは「データが悪い」という結論のほうだけ。

こちらが試した `accumulateSmalParentBend` が効かなかった理由も、これで説明がつく:

1. **`bendSmal = FromToRotation(smalRestDir, smalPosedDir)` は回転のうち
   ロール（ねじれ）を捨てている。**swing だけの回転を連鎖で合成しても、
   本来の連鎖は再現できない
2. **仮想 spine（joint 1-6）の寄与を `dev[joint] = dev[parent]` で捨てていた。**
   胴の曲げはここに乗るので、座位の主要成分をそのまま落としていた

**つまり「積む」方向は合っていたが、積む対象が足りていなかった。**

#### 次にやること

`smal_fk_points` が示すとおり、**標準の運動連鎖をそのまま計算すれば正しい姿勢が出る。**
Unity 側もそれに合わせる。`ApplyWorldRotation` のみ・IK 禁止の制約とは矛盾しない
（連鎖の計算結果を各ボーンの world 回転として書くだけ）。

**生成側への起票は不要。**データは正しい。

### 運動連鎖を積む実装と A/B（2026-09-10）

`accumulateSmalParentBend`（既定 OFF、バッチは `-accumBend true`）を、
**前回の版から作り直した。**

| | 前回（効かなかった版）| 今回 |
|---|---|---|
| 積むもの | `bendUnity`（swing のみ。ロールを捨てている）| **ローカル回転そのもの** `smalAccum[j] = smalAccum[parent] * local[j]` |
| 仮想 spine（1-6）| **飛ばしていた**（`dev[j] = dev[parent]`）| **通す。**胴の曲げはここに乗る |
| 使い方 | `tw = dev[parent] * bendUnity * restWorldRot` | `smalPosedDir = smalAccum[j] * smalRestDir`（標準形 `R[j] * rest方向` そのもの）|

#### 結果（座位の区間 f534-648）

`Docs/tmp/animal_chain_ab.png`。

| ボーン | 従来 | 連鎖を積む | 差 |
|---|---:|---:|---:|
| **LRPaw** | 80.0° | **51.0°** | **−29.0°** |
| **LRLo** | 60.0° | **35.0°** | **−25.0°** |
| **RRPaw** | 71.0° | **48.0°** | **−23.0°** |
| **RRLo** | 50.0° | **31.0°** | **−19.0°** |
| RFLo | 19.0° | 26.0° | +7.0° |
| RFPaw | 18.0° | 26.0° | +8.0° |
| 平均 | — | — | **−5.3°** |

**座位で実際に畳まれる後肢が、そろって 19〜29° 改善している。**
連鎖が要因なら効くはずのところに効いた、という形。
後肢 Upper（LRUp/RRUp 51°）が動かないのも整合する——**親が root なので積む相手がいない。**

全編では平均 −0.2° でほぼ中立。**立っている区間では連鎖の寄与が小さい**ので、これも整合。

#### 残っている別の問題（未着手）

`animal_chain_ab.png` を見ると、**モデルの向きが元動画と違う**（犬は左を向いているのに
モデルは奥／手前を向いている）。これは `globalOrient` と `modelOrientFix` の話で、
連鎖とは別系統。**姿勢が合ってきた分、向きのずれが目立つようになった。**
...
### 向きの調査（2026-09-10、途中・結論保留）

姿勢が合った後に残る「モデルの向きが違う」を追いはじめた。**2 つの調べ方が食い違っており、
まだ結論を出していない。**

#### 分かったこと

- **元動画の犬は右を向いている。**（当初「左向き」と書いたのは誤り。
  `Docs/tmp/animal_orient_check.png` で頭・口・尾に印を付けて確認した）
- **データの向きは正しい。**`body_pose` から組んだ骨格の頭・口・尾が実際の犬に乗る
- `rootYawFix` は **0° か 180° の二択**で、しかも一度決めたら固定（`rootYawFixDecided`）。
  実行ログでは `dot0=0.913 / dot180=-0.769` で 0° を選んでいる

#### 食い違っている 2 つの観測

| | 結果 |
|---|---|
| **ログの数値** | モデルの `-spine.forward` と データの体軸（pelvis0 → spine3）の差は<br>**全編 7.0°／座位 3.7°**（yaw の差 0.7°）。**向きは合っている** |
| **絵** | `Docs/tmp/animal_orient_zoom.png`。**モデルは背面から見えている**（尻と尾がこちら向き）。<br>犬は側面。**約 90° ずれて見える** |

`bindSpineW.euler=(0,0,0)`・`modelForwardLocal=(0.000,0.002,-1.000)` なので
`-spine.forward` が鼻先方向、というログの前提自体は成立している。

#### 次に確かめること

- **`modelForwardLocal`（メッシュから測った前方向）と spine ボーンの前方向が一致しているか。**
  ずれていれば「ボーンは正しく向いているがメッシュが別方向を向く」ことになり、
  両方の観測が両立する
- `AnimalBodyBasisResolver.TryResolveFromJoints` が返す `kpForward` の向きの規約。
...
### 頭を連鎖から外した（2026-09-10）— 全区間で最良の構成

ユーザー報告:「27 秒のやつ、犬は前を向いているのにモデルの頭が明らかに違う」。
頭に絞って測ったところ、**連鎖が頭だけを悪化させていた。**

#### `[ANIMALKP]` の Head 中央値

| | 従来 | 2 軸のみ | 連鎖+2軸 | **連鎖+2軸−頭** |
|---|---:|---:|---:|---:|
| 全編 | 39° | 41° | **53°** | **41°** |
| 座位 f534-648 | 29° | 8° | **39°** | **8°** |
| 27〜30 秒 | 105° | 97° | **135°** | **97°** |

**四肢の改善（LRUp 87° → 32°、LRUpTB 41° → 14°）は 2 軸だけで出ている。**
連鎖は四肢の遠位（LRLo / RRLo / LRPaw）を戻すのに効くが、頭には悪影響しかなかった。

理由: **頭だけ Unity 側の rest 方向が aim child ではなくメッシュ重心へのフォールバック**
（`SmalRestDirByJoint` の 16 のコメント）。frame map が他より不確かなので、
連鎖で入力を大きくすると誤差もそのぶん増える。

#### 全ボーンの平均（`[ANIMALKP]` 中央値の平均）

| 区間 | 従来 | 2 軸のみ | 連鎖+2軸 | **連鎖+2軸−頭** |
|---|---:|---:|---:|---:|
| 全編 | 31.1° | 30.2° | 30.3° | **29.3°** |
| 座位 | 36.4° | 30.9° | 27.9° | **26.0°** |
| 27〜30 秒 | 48.8° | 50.6° | 51.3° | **48.7°** |

**全区間で最良。**`excludeHeadFromChain` を既定 `true` にした（`-headChain true` で従来どおり連鎖に入れる）。

#### 27〜30 秒はまだ直っていない

`Docs/tmp/animal_head_27-30_now.png`。

**元動画の犬は伏せて手前を向いているのに、モデルは立ったまま横を向いている。**
`body_pose` から組んだ骨格は犬に重なっているので、**データは伏せを持っている。**
座位（17.8〜21.6 秒）は連鎖+2軸で大きく改善したのに、**この区間は改善しない。**

手がかり: この区間では**前肢が連鎖で悪化している**（LFLo 71° → 86°、LFPaw 74° → 88°）。
座位では前肢はほぼ不変だった。前肢の frame map か対応づけが、
この姿勢でだけ崩れている可能性がある。
（`[ANIMALKP]` の前肢の対応づけは 2026-08-28 のトポロジー復元と食い違っており、
**そもそも裏が取れていない**——この区間の前肢の数字は割り引いて読むこと。）

**次に見るのはここ。**

### 前肢の対応づけは正しかった（2026-09-10）— 2026-08-28 のトポロジー復元のほうが誤り

「`[ANIMALKP]` の前肢は D-007 の表と復元した木が食い違う」（2026-08-28）を決着させた。

#### まず: 26 点は 35 SMAL 関節の部分集合ではない

`pred_keypoints_3d` は "AniMer SMAL joint coordinates" と注記されているので、
`body_pose` から組んだ 35 関節と位置で対応づけを試みた。**最近傍の距離が体長の 13〜33%**
あり、対応が取れない。**betas（41 個）で骨長が変わる**のに rest skeleton で組んでいるため。
この経路では決まらない。

#### 剛体リンクで決めた

全点可視の 607 フレームで、`[ANIMALKP]` が使う 13 ペアの変動係数 `cv = sd/mean` を出した。

| ペア | cv | 平均長 | |
|---|---:|---:|---|
| LFUp kp12-kp8 | 0.0537 | 0.232m | 剛体 |
| LFLo kp8-kp14 | 0.0569 | 0.296m | 剛体 |
| **LFPaw kp14-kp3** | **0.2001** | 0.113m | **非剛体** |
| RFUp kp13-kp9 | 0.0572 | 0.231m | 剛体 |
| RFLo kp9-kp15 | 0.0641 | 0.291m | 剛体 |
| **RFPaw kp15-kp4** | **0.2197** | 0.114m | **非剛体** |
| LRUp kp7-kp10 | 0.0674 | 0.327m | 剛体 |
| LRLo kp10-kp16 | 0.0588 | 0.162m | 剛体 |
| **LRPaw kp16-kp5** | **0.1644** | 0.194m | **非剛体** |
| RRUp kp7-kp11 | 0.0642 | 0.329m | 剛体 |
| RRLo kp11-kp17 | 0.0582 | 0.158m | 剛体 |
| **RRPaw kp17-kp6** | **0.1541** | 0.202m | **非剛体** |
| Head kp18-kp24 | 0.0790 | 0.296m | 剛体 |

参考（明らかに剛体）: 肩どうし kp12-kp13 = 0.0349、尾付根-頭 kp7-kp18 = 0.0575。

**Up / Lo は全部 0.054〜0.067 で、既知の剛体と同等。対応づけは正しい。**

#### 2026-08-28 の復元が誤った理由

復元は `12-9`（0.349m, cv 0.0469）と `13-8`（0.343m, cv 0.0474）を選び、
`12-8`（0.232m, cv 0.0537）を落とした。**cv はわずかに低いが、長さが 1.5 倍ある。**
このクリップでは**左右の前肢がほぼ一緒に動く**ので、体を横切るペアも「剛体に見える」。
**cv だけで最小全域木を取ると、こういう長い横断リンクを拾う。**
上腕の長さ（0.23m）で見れば `12-8` / `13-9` が正しい。

**D-007 の対応表を採用し、8/28 の「前肢は誤っている可能性が高い」は撤回する。**

#### `Paw` 4 本は指標として使えない

kp3 / kp4 / kp5 / kp6 は**どの点とも剛体で結ばれていない**（最良の相手でも cv 0.09〜0.10、
距離 0.40m 以上）。**足先は推定が不安定**で、`[ANIMALKP]` の `*Paw` は
ノイズを相手に角度を測っている。**採否の判断に使わないこと。**

#### Paw を除いた比較

| 区間 | 従来 | 2 軸のみ | 連鎖+2軸 | **現行（−頭）** |
|---|---:|---:|---:|---:|
| 全編 | 29.6° | 27.9° | 27.8° | **26.6°** |
| 座位 | 32.3° | 25.1° | 24.8° | **22.1°** |
| 27〜30 秒 | 48.3° | 42.3° | 44.7° | **41.3°** |

**Paw を除いても現行が全区間で最良。**採用の判断は変わらない。

#### 残っている問題: 27〜30 秒の前肢下腿
...
**どの構成でも 69〜86° と大きく、連鎖でさらに悪化する。**
座位では LFLo 32°・RFLo 19〜26° なので、**この区間に固有の問題。**
27〜30 秒は犬が伏せて前肢が胸の下に隠れており、bbox も画面端で切れている。
**次に見るのはここ。**

### 前肢の副軸を首にした（2026-09-10）— 2 軸基底の縮退が原因だった

27〜30 秒の前肢下腿が 69〜86° だった件。**原因は 2 軸 jointFrameMap の副軸の選び方。**

#### 2 軸基底が前肢で縮退している

`SmalRollRefJoint` は「主軸 = 自分の rest 方向、副軸 = 対の関節の rest 方向」で基底を作る。
**2 本が平行に近いと基底が縮退してロールが決まらない。**

| 関節 | 副軸（旧）| なす角 | |
|---|---|---:|---|
| **LFUp / LFLo** | 互いに | **5.4°** | **縮退** |
| **RFUp / RFLo** | 互いに | **5.4°** | **縮退** |
| LRUp / LRLo | 互いに | 32.6° | 十分 |
| RRUp / RRLo | 互いに | 32.6° | 十分 |
| Head | Neck | 30.3° | 十分 |

**前肢は上腕と下腿の rest 方向がほぼ平行**（どちらも脚に沿って下を向く）。
後肢（32.6°）で 2 軸が効いて前肢で効かなかったのはこれ。

副軸を**首（15）**に替えると、前肢との角度は **83°** で条件が良い。

#### なぜ 27〜30 秒でだけ大きく出ていたか

ロールが決まらないと**曲げの平面**がずれる。ずれは rest からの振れ幅に比例して効く。

| 部位 | 全編 | 座位 | **27〜30 秒** |
|---|---:|---:|---:|
| LFLo の rest からの振れ | 26.1° | 25.0° | **45.0°** |
| RFLo | 28.1° | 9.6° | **46.9°** |
| Head | 25.1° | 16.4° | **66.5°** |

**振れ幅が大きい区間ほど誤差が大きい**という対応がそのまま出ている。

#### 結果（`[ANIMALKP]`、Paw は非剛体なので除外）

| 区間 | 従来 | 前まで | **前肢の副軸=首** |
|---|---:|---:|---:|
| 全編 | 29.6° | 26.6° | **23.6°** |
| 座位 f534-648 | 32.3° | 22.1° | **18.5°** |
| **27〜30 秒** | 48.3° | 41.3° | **30.2°** |
...
**絵**: `Docs/tmp/animal_frontroll_ab.png`。伏せの区間で前肢が前へ伸びて下がるようになった。

#### データは疑わなくてよいことも確認した

`body_pose` から組んだ骨の向きと `keypoints3d` の向きの差:
...
**データ内部は 27〜30 秒でも整合している。**86° はこちら側の誤差だった。

#### 残り

- **Head は 96° のまま**（27〜30 秒）。振れ幅 66.5° が最大で、かつ Unity 側の rest 方向が
  メッシュ重心へのフォールバック。**次はここ**
- RFUp が 12° → 27° に悪化。副軸変更の副作用か確認が要る

### 首と頭（2026-09-10）— 指標と絵が食い違う。判断はユーザー

ユーザー指摘 2 件:
1. 座位（f582/f630）で**上体と頭をつなぐ首が変**。犬はそうならない
2. 伏せ（f834/f882）で**モデルが左を向いている**。keypoint が悪いのか

#### 2 への回答: keypoints は正しい。頭の振りが適用されていない

| frame | 体軸（尾→頭）| 頭→鼻（body_pose）| 頭→鼻（keypoints）|
|---|---|---|---|
| 834 | **左・手前** | 右・手前 | 右・手前 |
| 882 | **左・手前** | 正面・手前 | 正面・手前 |

**犬は体を左に向けたまま頭だけカメラへ振っている。**`body_pose` と `keypoints3d` は一致。
モデルは体（左向き）は合っていて、**頭の振りだけが出ていない。**

#### 首にロール拘束を入れた（効果なし）

`SmalRollRefJoint` に `{15, 16}` を追加（首の副軸 = 頭。rest 方向のなす角 30.3°）。
「対になる joint が無い」として `FromToRotation` に落ちていたのを直したもの。

**数値も絵も変わらなかった**（Paw 除く平均 23.6° → 23.5°、Head 42° → 41°）。
**原理としては正しい拘束なので残すが、これが原因ではない。**

#### 首が折れて見える原因は「頭を連鎖から外していること」

`excludeHeadFromChain = true` だと、頭の世界回転は
**bind + 頭自身のローカル曲げ**で決まり、**首の回転を受け取らない。**
首は回るのに頭が追従しないので、**継ぎ目が折れる。**

`-headChain true`（頭も連鎖）で撮り直すと、
`Docs/tmp/animal_neck_headchain.png` の f834/f882 で
**頭がカメラの方へ振れており、元動画に近い。**

#### ところが指標は逆を言う

| Head の `[ANIMALKP]` | 頭は連鎖外（現行）| 頭も連鎖 |
|---|---:|---:|
| 全編 | 41° | 53° |
| 座位 | 8° | 38° |
| 27〜30 秒 | 97° | 135° |

| Paw 除く全体平均 | 頭は連鎖外 | 頭も連鎖 |
|---|---:|---:|
| 全編 | 23.5° | 24.6° |
| 座位 | 18.5° | 21.2° |
| 27〜30 秒 | 30.3° | 33.7° |

**指標は「外す」、絵は「入れる」を支持している。**

指標側を疑う理由はある: `[ANIMALKP]` の Head は
**モデル側が「頭ボーン → 頭メッシュの重心」（aim child が無いためのフォールバック）**、
**keypoint 側が kp18 → kp24（頭 → 鼻先端）**で、**別のものを比べている。**
他のボーンのように aim child どうしではない。

**採否はユーザーが絵を見て決める。**既定は現行（頭は連鎖外）のまま。

### 頭の照準を揃えた（2026-09-11）— 指標では測れないので絵で判断する

#### 変更

頭（joint 16）だけ、SMAL 側と Unity 側で**別のものを対応づけていた**。

| | 旧 | 新（`headAimFromModelForward`、既定 true）|
|---|---|---|
| SMAL 側 | 頭 → 口（joint 16 → 32）| **+X（rest skeleton の前方軸）** |
| Unity 側 | 頭ボーン → **頭メッシュの重心**（aim child が無いためのフォールバック）| **`worldFk0 * modelForwardLocal`**（コード内の証明 `visual_forward = rawWorldFk0 * modelFwdLocal` と同じ）|
| 副軸 | 首（15）| **前肢（7）**。+X と首は 6.6° しか離れず縮退するため |

#### `[ANIMALKP]` の Head では評価できない

変更しても数値がほぼ動かない（27〜30 秒で 97° → 99°）。**理由が分かった。**

`[ANIMALKP]` の Head はモデル側を `TryGetBoneDirectionForDiag`
→ `TryGetBoneCenterDirectionWorld` で取るので、**測っているのは今も
「頭ボーン → 頭メッシュの重心」**。こちらが照準に使う軸を変えても、
**指標は別の軸を見続ける。**
...
`Docs/tmp/animal_head_final.png`（27.8 / 28.6 / 29.4 秒。犬はカメラを見ている）。

| | 見え方 |
|---|---|
| 現行（頭は連鎖外）| 頭が横向きのまま。犬の「こちらを見る」動きが出ない |
| **案（照準を揃えて頭も連鎖）** | **頭が手前へ振れ、両耳が見える。元動画に近い** |

参考の数値（Paw 除く平均 / Head）:

| | 全編 | 座位 | 27〜30 秒 |
|---|---|---|---|
| 現行 | 23.5° / 41° | 18.5° / 8° | 30.5° / 99° |
| 案 | 24.5° / 52° | 21.2° / 38° | 33.5° / 133° |

**指標は現行を、絵は案を支持している。**上記のとおり指標は頭の照準を見ていないので、
**採否はユーザーが絵で決める。**既定は現行（`excludeHeadFromChain = true`）のまま。

#### 効かなかったもの（記録）

- 首（15）へのロール拘束追加（`{15, 16}`）… 数値も絵も変化なし。原理として正しいので残す
- 頭の照準の変更単体… 頭を連鎖から外したままでは、頭は自分のローカル回転しか受け取らないので効かない。**照準の変更は「頭も連鎖」と組でしか意味を持たない**

### 【訂正】頭の 2 つの変更はどちらも悪化させていた（2026-09-11）

ユーザー指摘:「頭の向き真逆だよね、どうして正しいと言える」。**そのとおり。**
`animal_head_final.png` で「頭が手前へ振れ、両耳が見える。元動画に近い」と書いたが、
**両耳が見えるのは正面ではなく背面。**絵の読み違いで、モデルは奥を向いていた。

#### 頭を測れる計測を入れた

`[ANIMALKP]` の Head はモデル側を**メッシュ重心方向**で測っており、
FK が照準に使う軸とは別物なので、頭の採否に使えない。

`[HEADAIM]` を追加した（`AnimalSmalFkApplier`、joint 16 の適用直後）:

```
[HEADAIM] headAimWorld=(x,y,z) bindDirLocal=(x,y,z) boneFwd=(x,y,z)
```

`headAimWorld = bone.rotation * bindDirLocal` は**適用後のボーンの照準そのもの**。
これを `body_pose` から組んだ頭の向き（joint 16 → 32）と比べる。

#### 結果: 元の状態が最良。私の変更は両方とも悪化

| 区間 | **旧照準・連鎖外（元）** | 照準=前方・連鎖外 | 照準=前方・頭も連鎖 | 旧照準・頭も連鎖 |
|---|---:|---:|---:|---:|
| 全編 | **61.6°** | 62.2° | 70.7° | 70.8° |
| 座位 | **17.9°** | 17.7° | 60.5° | 60.9° |
| 27〜30 秒 | **99.9°** | 107.5° | 152.0° | 129.3° |

- **頭を連鎖に入れると大幅に悪化**（座位 17.9° → 60.5°）。前回「絵では良く見える」と書いたのは誤読
- **照準を「モデルの前方」に揃える変更も悪化**（27〜30 秒 99.9° → 107.5°）

`headAimFromModelForward` を**既定 false に戻した。**`excludeHeadFromChain` は既定 true のまま。
**つまり頭は何も改善していない。**

#### 分かったこと（次の起点）

`[AIMBIND]` ログでリグの照準の決まり方を全ボーン出した:

```
bone=head  registered=none  resolved=head.001  children=3  bindDirLocal=(0.017, 0.681, -0.732)
bone=neck  registered=head  resolved=head      children=2  bindDirLocal=(0.000, 1.000, 0.000)
bone=front_l_upper registered=front_l_lower ... bindDirLocal=(0.000, 1.000, 0.000)
```

- **他のボーンは全部 `(0,1,0)`（ローカル +Y）** = 登録された aim child への方向
- **頭だけ `(0.017, 0.681, -0.732)`** で、しかも **aim child が登録されていない**
  （`RegisterAnimalAimPairs` に `head → ?` の組が無い）。
  first child の `head.001` にフォールバックしている

**頭だけ照準の出所が違う。**そのうえ 27〜30 秒で 99.9° ずれている。
`head.001` が本当に口の方向かは未確認。**次はここを確かめる。**

**頭は未解決。**

### 頭が直った（2026-09-11）— 照準はリグの流儀（ローカル +Y）に揃える

#### 原因

`[AIMBIND]` / `[HEADCHILD]` / `[HEADMESH]` を足して実測した。

**このモデルはスキンではなく剛体パーツの集合**だった。
Renderer の一覧が `body` / `head.001` / `er.L` / `er.R` / `arm.*` / `foot.*` / `tail.*` で、
**`head.001` は関節ではなく頭のメッシュ**。`er.L` / `er.R` は耳。

| ボーン | 登録 aim child | bindDirLocal |
|---|---|---|
| neck / 四肢 / 尾 | あり | **すべて `(0, 1, 0)`（ローカル +Y）** |
| **head** | **なし** | **`(0.017, 0.681, -0.732)`** |

**頭だけ aim child が登録されておらず**（`RegisterAnimalAimPairs` に `head → ?` の組が無い）、
first child の `head.001` の **bounds 中心**へ向く経路に落ちて、
**リグの流儀（+Y）から 43 度ずれていた。**
`head.001` の bounds は `extents=(0.0113, 0.0175, 0.0118)` で **+Y が最長**なので、鼻先も +Y 側。

#### 直し方

- **頭の照準をローカル +Y にする**（`headAimFromModelForward`、既定 true）
- **頭を連鎖に入れる**（`excludeHeadFromChain`、既定 false に変更）

**両方セットでないと意味がない。**照準だけ直すと相関が −0.066 に落ちる。

#### 結果（offset に依らない指標）

`[HEADAIM]` を追加した（適用後のボーンの照準そのもの `bone.rotation * bindDirLocal`）。
`[ANIMALKP]` の Head はメッシュ重心方向を測っており頭の評価に使えないため。

| 構成 | モデルの振れ sd | 範囲 | **相関** | 絶対差 中央 | **27〜30 秒** |
|---|---:|---|---:|---:|---:|
| 修正前 | 12.9° | 2〜55 | +0.365 | 61.6° | 99.9° |
| 照準=+Y のみ | 14.1° | 3〜55 | −0.066 | 64.3° | 79.0° |
| **照準=+Y ＋ 頭も連鎖** | **15.3°** | 4〜69 | **+0.746** | **42.1°** | **48.5°** |

データ側の振れは sd 23〜24°・範囲 0〜118°。**相関 0.365 → 0.746、27〜30 秒の誤差は半減。**

四肢も悪化しない（`[ANIMALKP]` Paw 除く平均: 全編 21.9°、座位 18.4°。
直前の構成は 全編 23.5°、座位 18.5° だったので微改善）。

絵: `Docs/tmp/animal_head_fixed.png`。27〜30 秒でモデルの頭がこちらへ振れるようになった。

#### 既定値を変えた

| | 値 |
|---|---|
| `headAimFromModelForward` | **true**（コード既定。シーンには serialize されていない）|
| `excludeHeadFromChain` | **false**（同上）|

**フラグ無しで走らせて検算済み**（頭 全編 41.6°／27〜30 秒 48.0°、明示指定の 42.1°／48.5° と一致）。

#### まだ残っていること

- **27〜30 秒の頭は 48.5° 残っている。**半減はしたが合ってはいない
- この区間は**データ側が伏せを表していない**（関節角で確認済み）ので、
  そもそも体が立ったままである点は変わらない
- 座位（f582）の体の向きは背面のままで、これは別系統

#### 途中で 3 回外した（記録）

1. 「頭も連鎖」だけ … 悪化（座位 17.9° → 60.5°）
2. 「照準＝モデルの前方（+X 対応）」… 悪化（27〜30 秒 99.9° → 107.5°）
3. 「照準＝前方を 36.9 度下げる」… 相関がさらに悪化（+0.365 → +0.234）

**いずれも「リグがどう組まれているか」を実測せずに幾何から推測したのが原因。**
`[AIMBIND]` で全ボーンの照準の出所を出したら 1 回で分かった。**先に実測する。**

### 【差し戻し】頭の修正は採用しない（2026-09-11）

実機で「首が正面になったが、めちゃくちゃで、首の傾きもまだおかしい」と指摘された。
**既定を元に戻した**（`headAimFromModelForward = false`、`excludeHeadFromChain = true`）。
...
**頭だけ `RegisterAnimalAimPairs` に組が無く**、first child の `head.001`（頭のメッシュ）の
**bounds 中心**へ向く経路に落ちている。リグの流儀（+Y）から 43 度ずれている。
**この欠陥自体は実在する**が、+Y に直しても見た目は良くならなかった。

#### 直前の数字は比較できなかった（訂正）

`[HEADAIM]` は `cache.bindDirLocal` を読む。**FK の中だけで照準を上書きした版では
キャッシュが古いままだったので、指標が古い軸を測っていた。**
「絶対差 42.3 度」と「78 度」は**別の軸を測った値で、比較できない。**
...
|---|---:|---:|---:|
| 修正前 | +0.365 | 61.6° | 99.9° |
| 照準 +Y ＋ 頭も連鎖（キャッシュ修正済み）| **+0.718** | **78.1°** | 54.2° |

**追従（相関）は上がるが、向きそのもの（絶対差）は悪化する。**
ユーザーの「ぐちゃぐちゃ」はこれと整合する。**採用しない。**

#### 効かなかったもの（記録）

- 副軸を耳（SMAL の LEar-REar と Unity の耳ボーン、どちらも主軸と 90 度）に変更 … 相関 0.746 → 0.752 でほぼ不変。**ロールは原因ではない**
- 耳から頭の座標系を組んで鼻先方向を出す … head ローカル −Z になり、**悪化**（相関 0.185）
- 首（15）の副軸 `{15, 16}` … 追加時も効果ゼロ、頭の変更後は悪化。**撤回した**

#### 残っている本質的な問題

**このリグには口・鼻のボーンが無い。**頭の子は `head.001`（頭のメッシュ）と左右の耳だけで、
**「鼻先はどちらか」をリグから決められない。**
SMAL 側は Head→Mouth という明確な軸を持つのに、Unity 側に対応する軸が存在しない。

考えられる筋:

1. **モデル側に muzzle/nose のボーンを足す**（またはアーティストに追加してもらう）
2. 頭のメッシュの**頂点**から鼻先を取る … このモデルはメッシュの Read/Write が無効で
   `vertices` が空。インポート設定を変えれば取れる
3. 頭の照準を**キャリブレーション値として手で与える**（モデルごとに 1 つのベクトル）

**3 が一番現実的**だが、モデルが増えるたびに人手が要る。
**次に決めるのはこの選択。**
...
| 試したこと | 結果 |
|---|---|
| 頭も連鎖に入れる（照準はそのまま）| 悪化（座位 17.9° → 60.5°）|
| 照準＝モデルの前方（+X 対応）| 悪化（27〜30 秒 99.9° → 107.5°）|
| 照準＝前方を 36.9 度下げる | 相関が悪化（+0.365 → +0.234）|
| 照準＝ローカル +Y ＋ 頭も連鎖 | 相関は上がるが向きは悪化。**実機で否定された** |
| 副軸＝耳 | 変化なし |
| 首の副軸 `{15,16}` | 効果ゼロ〜悪化。撤回 |

**共通する反省: 指標が何を測っているかを、変更のたびに確認していなかった。**
`[HEADAIM]` は `bindDirLocal` を読むので、照準を変えると**指標の基準そのものが動く。**
2 回それで誤った比較をした。

### 頭メッシュから鼻先を実測した（2026-09-11）— 既存の照準はほぼ正しかった

「モデルの頂点から鼻先を取る」案を実装した。`Assets/Editor/AnimalHeadAimBaker.cs`。

#### ツール

```
Unity.exe -batchmode -quit -projectPath <proj> -executeMethod AnimalHeadAimBaker.Bake           -logFile <log> [-prefab <path>] [-out <json>]
```

頭ボーン配下のメッシュ頂点を読み、**頭ボーン原点から最も遠い頂点**を鼻先とみなして
head ローカルの方向を出す。耳は名前で除外する。
**メッシュの Read/Write が無効だと `Mesh.vertices` が空になる**ので、
importer を一時的に読み書き可にして読み、`finally` で必ず元へ戻す。

#### 結果（00_Dog、頂点 200,562 個）

| | head ローカルの方向 | 実測との角度 |
|---|---|---:|
| **メッシュから実測した鼻先** | **(0.0073, 0.4704, -0.8824)** | — |
| 既存の `bindDirLocal`（bounds 中心）| (0.017, 0.681, -0.732) | **15°** |
| 私が入れた +Y | (0, 1, 0) | **62°** |

**既存の照準は鼻先から 15 度しか外れていなかった。**
つまり「Unity 側の照準が壊れている」という筋は**外れ**で、
**+Y にした私の変更が 62 度の誤りだった。**（前節で差し戻し済み）

#### 焼いた値で測り直すと全部改善する

| | 相関 | 絶対差 | 27〜30 秒 | 四肢 全編 | 座位 |
|---|---:|---:|---:|---:|---:|
| 修正前（既定の照準・連鎖外）| +0.365 | 61.6° | 99.9° | 23.6° | 18.5° |
| 焼いた鼻先・連鎖外 | +0.192 | 51.6° | 69.8° | 23.4° | 22.0° |
| **焼いた鼻先・頭も連鎖** | **+0.709** | **29.3°** | **43.1°** | **21.5°** | **18.4°** |

**絶対差 61.6° → 29.3°、27〜30 秒 99.9° → 43.1°。**
指標の基準（`bindDirLocal`）自体がメッシュ実測に接地したので、
**これまでのように基準が動いて比較が壊れることはない。**

**ここでも「照準の修正」と「頭を連鎖に入れる」はセットでないと効かない**
（照準だけだと相関が 0.192 に落ちる）。

絵: `Docs/tmp/animal_head_baked.png`。

#### まだ本採用していない

- **値が 00_Dog 決め打ち**。`AnimalPoseApplier.PrimeAnimalBind` にベタ書きしている。
  本採用するなら **52 体すべてを焼いて JSON から引く**必要がある
...
### 別モデル（36_LabradorDog）で確認（2026-09-11）— 首の問題は 00_Dog 固有

ユーザー要望:「00_Dog はメッシュが無いから固有の問題かもしれない。別の犬モデルで見たい。
全部共通で首が変なら首も調べるべき」。

#### 結果: 共通ではなかった

`Docs/tmp/animal_labrador.png`。

| | 首の見え方 |
|---|---|
| **36_LabradorDog 修正前** | **自然。**首から頭へ滑らかにつながっている |
| 00_Dog 修正前 | 首の継ぎ目が折れて見える（ユーザー指摘） |

**首がおかしいのは 00_Dog 固有。**00_Dog は**剛体パーツの集合**（`body` / `head.001` /
`er.L` / `er.R` / `arm.*` …）で、パーツどうしが別メッシュなので継ぎ目が露出する。
skinned モデルはメッシュが連続しているので、同じ回転でも継ぎ目が出ない。

**したがって「首も調べるべき」ではない。首の駆動そのものは壊れていない。**

#### そして頭の修正は Labrador では悪化する

| | 頭の照準（head ローカル）|
|---|---|
| 既定（フォールバック `Vector3.forward`）| (0.000, 0.000, 1.000) |
| ベイカーが焼いた値 | (0.279, 0.804, -0.526) |

焼いた値を当てると**頭がねじれて首が破綻する。**既定のほうが自然。

理由: **ベイカーの前提が rig によって成り立たない。**
Labrador の頭ボーンには子が 36 個あり（`head_attach` など）、
**口・鼻は頭ボーンではなく子ボーンに skin されている。**
「頭ボーンに重み 0.5 超で結びつく頂点のうち最も遠いもの」は口ではなく、
頭ボーン直近の小さな領域にしかならない（実測: 最遠でも 2.2mm）。

00_Dog では頭が 1 つの剛体パーツ（口を含む）なので、この前提が成り立っていた。

#### いまの結論

| | |
|---|---|
| 首 | **壊れていない。**00_Dog の見え方はパーツ分割によるもの |
| 頭のベイク | **00_Dog では効くが、skinned モデルには通用しない。**既定 OFF のまま |
| 汎用の解 | 未定。頭ボーン**とその子孫**に skin された頂点をまとめて見る、が次の案 |

#### 副産物（残す）

- `Assets/Editor/AnimalHeadAimBaker.cs` … 52 体中 51 体で鼻先候補を出せる。
  `.meta` はバイト列で退避して必ず戻す
- `Assets/Resources/animal_head_aim.json` … 焼いた表。キーは **prefab 名**
- `ReplaceableModel.sourcePrefabName` … インスタンスは `Track_<id>` にリネームされるので、
  どのモデルかを後から引く唯一の手段
- `BatchPlaybackLogger` の `-animalIndex` … バッチでモデルを切り替えられる

### 「Labrador の頭が正面を向かない」は、今のところ解けていない（2026-09-11）

ベイカーを 2 段階で直したが、**どちらも Labrador では悪化した。**

| 版 | Labrador の鼻先（head ローカル）| 結果 |
|---|---|---|
| 頭ボーンのみ | (0.279, 0.804, -0.526) | 頭がねじれる |
| **頭＋子孫**（口は子ボーンに skin されている）| 同上（頂点は 5339 → 10152 に増えたが値は不変）| 同じ |
| **＋ bindposes で変換**（skinned の頂点は bind 空間）| (-0.403, 0.824, -0.398) | やはりねじれる |
| 既定（フォールバック `(0,0,1)`）| — | **これが一番自然** |

`Docs/tmp/animal_labrador_head.png`。

#### 途中で直した実装の誤り（残す価値あり）

- **skinned の頂点は renderer の transform では変換できない。**bind 空間にあるので
  `mesh.bindposes[headBoneIndex].MultiplyPoint3x4(v)` で頭ローカルへ落とす
- 口・鼻が頭の**子ボーン**に skin されている rig があるので、頭ボーン単体では鼻先が取れない

それでも Labrador の最遠距離は **2.3mm** にしかならない。頭の実寸に対して 2 桁小さく、
**まだ何かを取り違えている。**（値は正規化して使うので向きだけは出るが、信用できない）

#### 何が本当の壁か

Labrador の頭は `[AIMBIND]` で **`gotDir=False`**——
`RegisterAnimalAimPairs` に `head -> ?` が無く、`head_attach` にも Renderer が無いため、
`PrimeAnimalBind` の**既定値 `Vector3.forward` がそのまま使われている。**
つまり**照準が「たまたま +Z」という任意の軸**で、そこから作った `jointFrameMap` に
意味は無い。にもかかわらず、**メッシュから出した「正しそうな軸」に替えると絵は悪化する。**

これは、頭の対応づけが**軸 1 本では決まらない**ことを示している。
`jointFrameMap` は主軸＋ロールの 2 自由度を要求するのに、
頭には Unity 側で信頼できる副軸が無い（耳を試したが変化なし）。

#### 現実的な解（未着手・要判断）

**モデルごとに頭のフレームを手で較正する。**軸 1 本ではなく、
「rest でこの向き・このロール」という**回転そのもの**を人が合わせて保存する。
自動で出そうとして 4 回失敗しているので、**実験で使うモデルだけ手で合わせる**のが現実的。

52 体すべてではなく、**実験に使う数体**なら手作業で足りる。

#### いまの既定（採用済みのみ）

| フラグ | 既定 | 状態 |
|---|---|---|
| `accumulateSmalParentBend` | **true** | 採用（座位 32.3° → 22.1°）|
...
### 当てはめても正面を向かなかった — 原因は「当てはめは連鎖前提、runtime は連鎖なし」（2026-09-11）

#### 絵の判定

`Docs/tmp/animal_head_fit2.png` / `animal_head_fit2_zoom.png`。
**27〜30 秒で元動画の犬は真正面（両目・鼻がこちら向き）なのに、当てはめ後も左向きの横顔のまま。**
数値は 95.9° → 44.3°（00_Dog）、90.6° → 43.6°（Labrador）と半減しているが、
**絵は直っていない。**44° はまだ大きい。

#### データ側は正しかった

「そもそも元データが悪いのでは」を潰すため、**`keypoints3d` と `body_pose` を
カメラ空間で突き合わせた**（scratchpad `head_data_check.py`。投影は
`animal_three_panel.py` の実装をそのまま使う）。

| 区間 | kp 鼻と smal 口の差（中央） | 頭の振り kp | 頭の振り smal | 鼻の z（kp） |
|---|---:|---:|---:|---:|
| 全編 | 21.9° | 24.3° | 48.8° | -0.84 |
| **27〜30 秒** | **21.7°** | **68.5°** | **68.0°** | **-0.97** |

- カメラ空間は x 右 / y 上 / z 前。**鼻の z = -0.97 は「ほぼ真っ直ぐカメラを向いている」**。元動画のとおり
- **頭の振り（体軸と鼻のなす角）は kp 68.5°・smal 68.0° で一致**。body_pose も同じ振りを持っている
- 2 つのデータ源の差は全区間で 21〜24° とほぼ一定。定義差（kp は 頭 18 → 鼻 24、SMAL は 頭 16 → 口 32）で、当てはめの自由度が吸収できる分

**つまりデータは頭の振り 68° を持っている。落としているのは受け取る側。**
...
```python
chain = []; k = 16
while k > 0: chain.append(k); k = PAR[k]      # 16 から root まで積む
posed = Aacc @ SMAL_HEAD_REST                 # ← 連鎖を積んだ回転
```

を前提に `bindDirLocal` と `θ` を出した。ところが runtime は

```csharp
bool useChain = accumulateSmalParentBend && !(excludeHeadFromChain && joint == 16);
Quaternion smalPose = useChain ? smalAccum[joint] : smalLocal;   // 頭は smalLocal
```

で **`excludeHeadFromChain = true` のため頭だけ自分のローカル回転しか使わない。**
頭の振り 68° は首・背骨に乗っているので、ローカル回転だけでは出てこない。

これは直前に直した「当てはめは `FromToRotation` 前提、runtime は耳ベースの 2 軸基底」と
**まったく同じ種類の食い違い**（当てはめた式と実行する式が違う）。
なお本ドキュメント上部の 2026-09-11 の記述に
「照準の変更は『頭も連鎖』と組でしか意味を持たない」と既に書いてあった。

#### オフラインでの予測（Unity を回す前に確認）

`[HEADFIT]` の `restWorldRot` は `worldFk0 * boneBindWorld` で連鎖フラグに依存しないので、
**当てはめ直さずに `posed` だけ差し替えて予測できる**（scratchpad `head_chain_predict.py`）。

| モデル | posed | 全編 | 27〜30 秒 |
|---|---|---:|---:|
| 00_Dog | local（今の runtime）| 27.7° | **43.9°** |
| 00_Dog | chain | 21.3° | **11.9°** |
| 36_LabradorDog | local（今の runtime）| 27.9° | **42.1°** |
| 36_LabradorDog | chain | 18.6° | **7.9°** |

**local 版が実機バッチの実測 44.3° / 43.6° をほぼ再現している**ので、
このオフライン模型は忠実で、chain 版の数字は予測として信用できる。

**注意**: 「頭も連鎖」は過去に失敗として退行させた案だが、**そのときは照準が未当てはめで
2 軸基底（耳）と併用していた**。今回は照準・ロールを当てはめ済みで頭は `FromToRotation` 固定。
同じ名前でも別の構成。

#### Unity で確認（頭も連鎖に戻す）

`-headAim true -headChain true`。分岐が本当に切り替わったかは `[HEADFIT]` に
`useChain=` を足して確認した（fitE_dog 102 行・fitE_lab 108 行すべて `True`）。
「直したのに効かない」と「分岐が切り替わっていない」を取り違えないため。

指標は `[HEADAIM]`（適用後の頭ボーンの照準そのもの）と `keypoints3d` の 頭18 → 鼻24 の角度差。
測定コード scratchpad `headaim_measure.py`。過去の実測値を再現することで検算している。
...
オフラインの予測（11.9° / 7.9°）とほぼ一致した。

**頭以外は動いていない**（このフラグは joint 16 にしか掛からない）。
`[ANIMALKP]` の Paw 除外平均（四肢）:

| | 当てはめのみ | 当てはめ+連鎖 |
|---|---:|---:|
| 00_Dog 全編 / 27〜30 秒 | 23.0° / 23.5° | 23.1° / 23.5° |
| 36_LabradorDog 全編 / 27〜30 秒 | 42.7° / 47.8° | 42.8° / 47.8° |

`[ANIMALKP]` の `Head`（**別指標**。ボーンではなくメッシュ重心の方向を測る）も
00_Dog で 45.0° → 23.0°（全編）、56.0° → 34.0°（27〜30 秒）と改善した。
Labrador はこの項目が `novis` で出ない。

絵: `Docs/tmp/animal_head_chain.png`（頭の拡大、修正前 / 当てはめのみ / 当てはめ+連鎖）、
`Docs/tmp/animal_head_chain_3panel_dog.png` / `_lab.png`（元動画 / キーポイント / モデル）。

#### 既定値

| フラグ | 変更 |
|---|---|
| `excludeHeadFromChain` | true → **false**（頭も連鎖に入れる）|
| `headAimFromModelForward` | false → **true**（当てはめ表の入口）|

**2 つはセットでしか意味を持たない。**当てはめが連鎖前提で解かれているので、
片方だけ入れると式が食い違う（今回の 44° はまさにそれ）。
シーンにはこの 2 つは serialize されていないので C# の既定値が効く
（`useTwoAxisJointFrameMap` だけは TestScene / TrialScene に 1 で焼いてある）。

**残る課題**: 27〜30 秒でモデルは立ち姿、実写は伏せ。これは頭とは別で、
この区間の body_pose 自体が屈曲不足（後膝 111/136° に対し座位 122/123°・rest 147°）。
生成側の問題として D-00x に出す。Labrador の四肢 42.8° が 00_Dog の 23.1° より
大きいのもモデル差として別途。

#### 切り出しの注意（可視化）

頭の拡大は「モデル外接矩形の左 45% × 上 60%」で切っていたが、
**前肢が頭より前に出るフレーム（f834）で頭が枠外**になり草だけのコマが並んだ。
`headzoom2.head_crop` に変更: マスクの**上半分で最も前（左）にある点**を鼻先とみなし、
その周りを正方形で切る。割合ではなく中身に合わせる。

#### 既定値を変えるときの落とし穴（2026-09-11 に踏んだ）

`excludeHeadFromChain` / `headAimFromModelForward` は **2 か所に同名フィールドがある**。

```
StreamingStereoVideoPlayer.Core.cs          ← こちらが実効値（Inspector に出る）
  ↓ PosePipeline.partial.cs:236-237 が毎フレーム代入
AnimalSmalFkApplier                          ← 毎フレーム上書きされる
```

applier 側の既定だけ変えて既定値のみのバッチを流したら `useChain=False` のまま、
数値も修正前（97.9°）に戻っていた。**プレイヤー側の既定も変える。**

`-headChain` / `-headAim` はプレイヤー側の setter を呼ぶので、バッチ引数では正しく効く。
このため「引数付きは直っているのに既定は直っていない」という食い違いが起きる。

プレイヤー側も直したうえで**引数なし**のバッチで確認した（既定値だけで同じ経路を通るか）。

| | useChain | 全編 | 27〜30 秒 |
|---|---|---:|---:|
| 00_Dog `-headAim -headChain` 付き | True | 18.9° | 11.3° |
| **00_Dog 引数なし** | **True** | 20.1° | **12.0°** |
| 36_LabradorDog `-headAim -headChain` 付き | True | 19.1° | 6.5° |
| **36_LabradorDog 引数なし** | **True** | 19.7° | **6.2°** |

差は診断ログの間引き位置によるフレームの取り方の違い（n=6 と 7）。

#### 新経路は当てはめ表に載っているモデルだけに掛ける（2026-09-11）

既定を ON にした直後の状態には**取りこぼしがあった**。当てはめ表
（`Assets/Resources/animal_head_fit.json`）に載っているのは 2 体だけなのに、
`excludeHeadFromChain = false` は**全 52 体に効く**。表に無いモデルは

- 照準 `bindDirLocal` … 頭メッシュの重心のまま（鼻先ではない）
- ロール θ … 0（拘束されない）
- `jointFrameMap` … `headAim` が立つと 2 軸基底（副軸 = 首）から `FromToRotation` へ変わる

という、**当てはめの前提が無いまま連鎖と写像だけ変わった**状態になる。
連鎖は入力を大きくするので、写像が不確かなモデルでは誤差もそのぶん増える。

そこで `AnimalPoseApplier.HasBakedHeadFit(cache)` を足し、
**表に載っているモデルだけ**が新経路（連鎖 + `FromToRotation` + ロール）に入るようにした。
載っていないモデルは従来経路（頭は連鎖から外す + 首を副軸にした 2 軸基底）のまま。

確認（`[HEADAIM]` と keypoints3d の 頭18 → 鼻24、27〜30 秒の中央値）:

| | useChain | 全編 | 27〜30 秒 |
|---|---|---:|---:|
| 00_Dog（表にある、既定のみ）| True | 19.6° | **11.3°** |
| 27_GermanShepherd（表に無い、既定のみ）| False | 33.4° | 96.2° |
| 27_GermanShepherd（`-headAim false -headChain false`）| False | 32.9° | 96.6° |

表に無いモデルは**明示的に旧設定で走らせたときと一致**する（退行なし）。
