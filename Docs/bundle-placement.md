...
## 再生中にモデルを替えると大きさが変わる（2026-08-31 実測）

### 症状

`bundle_human` を見ている途中でキャラクターを替えると、見かけの大きさが大きく変わり、
そのまま戻らない。

### 原因: ⑨ の測り直しが「たまたま走ったフレームの姿勢」を焼き込んでいる

`RefineLockedScaleFromProjectedBones`（⑨）は track ごとに**一度だけ**走り、その瞬間の

```
ratio  = 骨の投影高 / bbox 高
refined = locked × (1.0 / ratio)
```

をロック値に焼き込む。ガードは `ratio` を 0.4〜3.0 まで許すので、**最大 ±3 倍**まで動きうる。

モデルを替えると `RecreateTrackInstanceForModelSelection` が
`lockedModelLocalScaleByTrack` を消し、`GetOrLockModelLocalScale` が再ロック時に
`scaleRefinedByTrack` も消す。**状態のリセット自体は正しい。**
問題は、その結果 ⑨ が「差し替えた瞬間のフレーム」でやり直されること。

### 決定的な対照実験: 同じモデルに差し替えても縮む

`-swapModel "0:600:0"`（index 0 → index 0、**まったく同じモデル**）で実測。

```
[SCALEFIX] track=0 boneRatio=0.928 target=1.000 scale 0.2399 → 0.2585 (×1.077) bboxH=441
[SWAP] f=600 track=0 modelIndex=0 に差し替え
[SCALEFIX] track=0 boneRatio=1.092 target=1.000 scale 0.2399 → 0.2197 (×0.916) bboxH=176
```

モデルが同一でも **0.2585 → 0.2197（15% 縮小）**。したがって差の出どころはモデルではなく、
**測り直しが走ったフレームの姿勢**だけ。shot 先頭では人が立っていて `boneRatio=0.928`
（7.7% 拡大）、f=600 では別の姿勢で `boneRatio=1.092`（8.4% 縮小）。

別モデル（index 8）でも同じ挙動で、0.2585 → 0.2162（16.4% 縮小）。
ロックの基準となる `desiredScale` は両方とも 0.2399 で一致しており、
`TryResolveShotStartScaleReference` が shot 先頭に固定されている側は正しく働いている。

位置は無関係だった。`bottomDelta` の差は 0.03px 以下で、動いているのはスケールだけ。

### 併発: 差し替えなしでも run ごとに 3% ぶれる

差し替えを一切しない 2 つの run でも、⑨ が焼き込む値は 0.2502 と 0.2585（3.3% 差）だった。
⑨ は「条件が最初に揃ったフレーム」で走るため、バッチ再生のフレーム巡りが変わると
測る姿勢も変わる。**もともと決定論的でない。**

### 方針: 補正倍率を track の性質として持ち越す

⑨ の出力の意味は「② の式がこの track の姿勢に対してどれだけ過小／過大に出るか」であり、
**モデルインスタンスではなく track の性質**。したがって

- 倍率（`refined / locked`）を track ごとに保持する
- モデル差し替えで再ロックするときは、測り直さずその倍率を掛け直す
- shot 境界では従来どおり破棄して測り直す（カットが変われば典型姿勢も変わる）

とすれば、モデル差し替えは大きさに対して中立になる。


### 対処と実測（2026-08-31）

`scaleRefineFactorByTrack` に倍率を保持し、`GetOrLockModelLocalScale` が再ロックするときに
倍率があればそれを掛けて確定させる（測り直しをスキップ）。`ResetPerShotTrackState` は
ロック・`scaleRefinedByTrack` と一緒にこの倍率も捨てる。

修正後、`-swapModel "0:600:8"` で差し替えても

```
[SCALEFIX] track=0 boneRatio=0.928 target=1.000 scale 0.2399 → 0.2585 (×1.077) bboxH=441
[SWAP] f=600 track=0 modelIndex=8 に差し替え
[SCALEFIX] track=0 補正倍率 x1.077 を持ち越し（測り直さない）
```

| frame | 修正前 | 修正後 |
|---|---|---|
| 0〜570 | 0.2585 | 0.2585 |
| 600（差し替え） | **0.2162** | 0.2585 |
| 630〜1350 | 0.2162 | 0.2585 |

再生全体で `scale` に現れる値は `0.2585` の 1 つだけになった。

shot 境界での測り直しは従来どおり動く。`bundle_animal`（shots=28）で
測り直し 23 回・持ち越し 0 回（差し替えていないので当然 0）。

EditMode テストは 517 件中 492 passed / 25 failed で、失敗リストはベースラインと一致。

### 残る既知の性質: ⑨ はもともと決定論的でない

差し替えを一切しない 2 つの run でも、⑨ が焼き込む値は 0.2502 と 0.2585（3.3% 差）だった。
「条件が最初に揃ったフレーム」で走るため、再生のフレーム巡りが変わると測る姿勢も変わる。
今回の修正は**差し替えによる跳ね**を消すもので、この 3% のばらつきは別問題として残る。
消すなら「shot 先頭フレームで必ず測る」形にする必要があるが、そのフレームまで
シークしないと FK 後の姿勢が得られないため、実装は別途検討。

## Screen Dist を動かすとモデルだけが取り残される（2026-09-05 実測）

### 症状

設定の Screen Dist を動かしても「距離が変わっていない」ように見える。スクリーンも
Else（ボール）もその場にあり、Human だけが挙動を変える。

### 測ったもの

`bundle_human.svb` をスクリーン 1.0m と 3.0m でバッチ再生（各 45 秒）。
`[DEPTH8]` `[ROOTDIAG]` `[PLACE]` `[SCALEFIX]` を突き合わせた。

| | 1.0m | 3.0m | 比 |
|---|---:|---:|---:|
| anchorZ（配置深度・メートル） | 0.8625 | 2.8595 | ×3.32 |
| ロックされたスケール | 0.2585 | 0.8571 | ×3.32 |
| ⑨ の boneRatio（補正後） | 1.000 | 1.000 | — |
| **⑧ が見る深度（root）** | 0.642 | **0.408** | **×0.64** |
| ⑧ が使った倍率 | 1.296 | **6.403** | — |
| ⑧ 後の boneRatio | 0.87〜1.06 | **0.54〜0.77** | — |

**配置とスケールは正しく比例している。**壊しているのは ⑧ だけ。

### 原因: root は体の位置ではない。ずれ幅は screenDist とともに増える

`instance.transform.position`（root）は体から手前にずれている。
ずれ幅は **観測上** `anchorZ × lossyScale` に一致する（下の計算）。
**これは当てはまる関係式であって、機構の説明ではない。**
`[HIPSTAGE]` で ① の配置直後に既にずれていることが判っており（後述）、
anchorZ がずれを作っているわけではない。

```
1.0m:  2.8595 → ずれ 0.222m   root 0.642 + 0.222 = 0.864 = anchorZ 0.8625  ✓
3.0m:  0.8625 → ずれ 2.451m   root 0.408 + 2.451 = 2.859 = anchorZ 2.8595  ✓
```

ずれの実体は `localHips.z`（root ローカルでの hips 位置）で、run 内では完全に一定
（1.0m で `(-0.01, 1.02, 0.86)`、3.0m で `(-0.03, 1.07, 2.86)`）。X と Y はほぼ同じで
**Z だけが anchorZ の差ぶん（+2.00）動いている**。リグ自体は剛体で、
`(骨格の奥行き span) / scale − localHips.z` は両 run とも +0.55 で一致する。
つまり体の内部構造は正常で、**root だけが体から離れている**。

**未解明: なぜ localHips.z が anchorZ のメートル値と一致するのか。**
剛体のローカル offset が screenDist に依存するのはおかしい。次の経路は潰した:

- ボーンへの `localScale` 書き込み → 無い（`TryApplyHumanBoneLengthCorrection` はログのみ）
- Hips への `position` 書き込み → 無い
- `AlignHumanoidHipsToSmplRoot` / `GetSmoothedSmplRootWorld` → どちらも world で正しく計算している

`[HIPSTAGE]` で **① の配置直後に既にずれている**ところまでは判った（上表）。
ローカル offset を変えうる経路（bone の `localScale` / `position` 書き込み、
`AlignHumanoidHipsToSmplRoot`、`GetSmoothedSmplRootWorld`）は全て潰してあるので、
残るのは prefab の bind pose か FK の回転が hips より上のノードに掛かっている場合。
**⑧ の実害は無くなったので、ここは追わずに残す。**

**訂正（同日、`[HIPSTAGE]` 実測後）: ④ `AlignModelBodyToAnchorDepthIfEnabled` は犯人ではない。**
段ごとに測ったら ④ は完全な no-op だった（下表）。ずれは ① の配置時点で既にあり、
② の姿勢適用（`AlignHumanoidHipsToSmplRoot`）が **root を引き戻して体を anchorZ に載せている**。
つまり root が体の手前に残るのは姿勢適用の正常な結果で、問題は
**その後の段が `transform.position` を「モデルの深度」として使っている**こと。

| 段 | 1.0m rootZ / bodyZ | 3.0m rootZ / bodyZ | gap |
|---|---|---|---|
| `1_placed`（① 配置直後） | 0.878 / 1.101 | 2.865 / 5.319 | +223mm / +2454mm |
| `2_skeleton`（② 姿勢適用後） | 0.652 / 0.875 | 0.416 / 2.870 | 変わらず |
| `3_bodyalign`（④ 後） | 0.652 / 0.875 | 0.416 / 2.870 | **no-op** |

`2_skeleton` で **体が anchorZ（0.878 / 2.865）に載る**のが読み取れる。姿勢適用は正しい。

### 誰が 6.4 倍を作ったか: 順序クランプ

`RefineDepthFromProjectedBones`（⑧）の生の比はガード
`MaxProjectedBoneRatioForScaleRefine = 3.0` を超えると弾かれる。**ログの 6.403 は
その上限を超えている**ので、投影から出た値ではありえない。作っているのは
`ClampRatioPreservingOtherOrder`:

```csharp
lower = Mathf.Max(lower, (other.anchorZ + eps) / (currentZ * k));
```

`currentZ` は **root の深度**（0.408）、`other.anchorZ` は Else の配置深度（2.72m、単位は
メートルで正しい）。root が体の 2.45m 手前にいるので「人がボールより手前すぎる」と
誤判定し、`2.72 / 0.408 = 6.67` を下限として押し上げる（実測 6.403、平滑化ぶん）。

⑧ はその倍率を **root の位置ベクトル**に掛ける。root は 0.408 → 2.837（screenDist で
クランプ、発動 21.4%）へ動き、剛体でぶら下がっている体は **2.86m → 5.30m** へ押し出される。
スクリーンより奥。

boneRatio の実測 0.54 は `2.86 / 5.30 = 0.540` と一致する。

### なぜ screenDist を上げるほど悪化するか

root と体のずれは実測で 1.0m の 0.22m から 3.0m の 2.45m へ **×11** に増える
（scale の増加 ×3.32 だけでは説明できない。機構は未解明）。
一方 root の深度は逆に小さくなる（0.642 → 0.408）ので、順序クランプが要求する倍率は
急激に跳ね上がる。既定 2.0m 付近で目立たないのはずれがまだ小さいから。

### 結論

- **配置・スケール・`[SCALEFIX]`（スケール再ロック）に欠陥はない。**Screen Dist に
  きちんと追従している（boneRatio 0.928 → 1.000 は両距離で同一）
- **ただし `[DEPTH9]`（Else を人の骨格基準で置く段）は別。**これは壊れた入力を食っている。
  3.0m で `skelZ=5.2203`（⑧ が押し出した後の体）を基準に球を置き、`final=2.9999` と
  screenDist のクランプに張り付いていた。ユーザーの言う「ボールもその場にいる」はこれ。
  ⑧ を直せば入力が正しくなるので、ここは追随して直るはず（要確認）
- **⑧ とその順序クランプが `transform.position` を体の位置とみなしているのが原因**
- ④ を消して直すのは筋が悪い。④ は「hips を anchorZ に置く」ための正しい処理で、
  ② がスケールを決める前提そのもの

### 測定に使ったスクリプト

[docs/log-analysis/screendist_depth_check.py](log-analysis/screendist_depth_check.py)。
`python screendist_depth_check.py <1.0m のログ> <3.0m のログ>` で
`[DEPTH8]` `[HIPSTAGE]` `[ROOTDIAG]` `[PLACE]` をまとめて出す。

修正前の基準値（`bundle_human.svb`、45 秒）:

| | 1.0m | 3.0m |
|---|---:|---:|
| ⑧ の倍率 | 1.296 | 6.403 |
| クランプ発動 | 0.6% | 21.4% |
| `[PLACE]` boneRatio 中央 | 0.980 | **0.663** |
| 0.87〜1.06 の外 | 22.6% | **97.7%** |

### 対処（2026-09-05）

⑧ の深度を **root ではなく体（Humanoid の Hips）で解く**ようにした。
`ResolveDepthReferenceWorld` を追加し、Hips が取れないモデル・カテゴリ
（Animal は Generic リグで ④ を通らないので root と体がずれない）は従来どおり root を返す。

- 順序クランプにも体の深度を渡す
- `screenDist` のクランプも体に掛ける（root に掛けると root だけが止まり、
  体は 3.0m 設定で 5.45m に残る）
- 位置ベクトルを倍率で伸ばす形をやめ、**体の移動量ぶんだけ root を平行移動**する。
  伸ばす形が正しいのは「その位置に実体がある」ときだけで、体から離れた root に
  掛けると横方向にも飛ぶ

合格条件は上のスクリプトの docstring に書いた。

### 修正後の実測（2026-09-05）

| | 1.0m | 3.0m |
|---|---:|---:|
| ⑧ が見る深度 | 0.875（anchorZ 0.878）| 2.870（anchorZ 2.865）|
| 体 − root | +223mm | +2454mm |
| ⑧ の倍率 | 1.296 → **1.174** | 6.403 → **1.164** |
| **shot 先頭 boneRatio** | 0.869 → **1.009** | **0.536 → 1.050** |
| 全区間 boneRatio 中央 | 0.980 → 1.072 | 0.663 → **1.128** |
| ボール `[DEPTH9]` final | 0.989（移動 161mm）→ 0.902（67mm）| **3.000 に張り付き**（188mm）→ 2.925（92mm）|

**Screen Dist で結果が変わらなくなった。**修正前は shot 先頭 boneRatio が 0.869 対 0.536 と
距離で 1.6 倍違ったが、修正後は距離を通して 1.0 付近に揃う。ボールも screenDist の
クランプから外れた。

| screenDist | shot 先頭 boneRatio（修正後）|
|---|---:|
| 1.0m | 1.009 |
| **2.0m（既定）** | **1.019** |
| 3.0m | 1.050 |

**Animal に退行なし。**`ResolveDepthReferenceWorld` は Humanoid でなければ root を返すので
Animal は従来経路のまま。実測でも shot 先頭 1.000（修正前 1.000）、全区間 1.011（同 1.060）。

全区間中央値 1.07〜1.13 は**設計どおりの範囲**（⑨ が合わせるのは shot 先頭の 1 フレームだけで、
その後は姿勢で 1.0〜2.2 動く。全区間 1.2 前後が想定値）。
評価は [docs/log-analysis/shotstart_boneratio.py](log-analysis/shotstart_boneratio.py) で
shot 先頭を見ること。**全区間中央値を 1.0 と比べてはいけない**（過去に 3 回誤った）。

### 残っている性質: screenDist のクランプが 63〜74% のフレームで効いている

⑧ の解は 1.045（1.0m）/ 3.348（3.0m）で、どちらも `screenDist` を超える。
つまり **⑧ は体をスクリーンより奥へ置きたがっている**が、クランプで止められている。
止められたぶん体が手前に残り、boneRatio が 1.07〜1.13 と 1.0 より大きい側に出る。

- 修正前の 1.0m で全区間 0.980 と良く見えていたのは、**root だけをクランプしていたので
  体がスクリーンの奥（1.051m）へ抜けられていた**ため。偶然
- スクリーンは Background キュー + ZWrite Off でモデルを遮蔽しない（[docs/adr](adr/)）ので、
  奥へ置いても消えはしない。クランプの根拠は描画ではなく飛び出し量の設計側にある

**この上限は緩めない（2026-09-05 判断）。**設計上の判定基準である shot 先頭 boneRatio は
1.009 / 1.019 / 1.050 と通っており、全区間 1.07〜1.13 も想定の 1.2 以下。緩めれば全区間は
1.0 へ寄るが、⑧ が姿勢のノイズを深度で吸収するようになり、平滑化のコメント群が防ごうと
していた「モデルが前後に泳ぐ」状態を招く。

### 残っている欠陥: ⑨ が食う人の深度がクランプの値のまま

`[DEPTH9] skelZ` は修正後も **1.000 / 3.000 とちょうど screenDist に張り付いている**。
⑨ は Else を人の骨格深度から相対配置するので、**基準が体の実深度ではなくクランプの
産物**になっている。ボールの移動量が 188mm → 92mm に減ったのは入力の壊れ方が
小さくなったからで、この結合自体は直っていない。ユーザーの言う
「ボールもその場にいる」の残り半分はここ。

## バッチで実機を再現する手順（2026-09-09 確立）

**この 3 つを揃えないとバッチは実機の代用にならない。**2026-09-08〜09 に、
揃っていないまま一日測って何度も誤った結論を出した。

```
run_cap.ps1 -Tag <名前> -Bundle bundle_human.svb -Seconds 60   -Extra "-diagLogs true -diagEveryN 1 -targetFps 72 -remember true -screenDistance 1.0"
```

| | 揃えるもの | なぜ要るか |
|---|---|---|
| `-targetFps 72` | **Update の回数** | バッチは約 465fps。⑧ は毎 Update 反復するので、揃えないと収束度が 6 倍違う（15.5 対 2.4 tick/フレーム）|
| `-remember true` | **モデルの選択** | バッチの既定は `remember=False`。実機が選んだモデルを読まない |
| `-screenDistance 1.0` | **画面までの距離** | `run_cap.ps1` は 2.0m 決め打ち。実機の設定値を使うこと |

### 一致の確認（f1040、`bundle_human`、`01_Female_A_02`）

| | scale | 深度 | 描画高 | boneRatio |
|---|---:|---:|---:|---:|
| 実機 | 0.2585 | 0.926 | 510 | 1.008 |
| **バッチ（上の設定）** | **0.2585** | **0.925** | **511** | **1.008** |

### 揃えられないもの

- **ステレオ**（バッチは片目）
- **実機のコマ落ち**（実機ログの `[FPS]` で確認できる。今回は 71.0〜72.1 で安定）

### 設定値は推測せずログに出す

`logDeviceDiagnostics` を有効にすると `[SET]` が出る（screenDist / fovx / 各フラグ）。
2026-09-09 に Screen Dist の違いを推測で追いかけて時間を使った。
ユーザー指摘「疑うなら調べればいいのでは、ログあるなら」。**まずログに出す。**

### 測り終わったら

`%USERPROFILE%\AppData\LocalLow\DefaultCompany\stereoCrafter\model_selection.json`
を置いたままにしない（以後のバッチ全部に効く）。

## 「36 秒で急に大きくなる」の原因は `anchor_z`（2026-09-09 実機ログで特定）

実機で「36 秒で急に大きくなった」と報告された区間を、実機ログで直接追った。

### 実機の数値（`bundle_human`、`01_Female_A_02`、Screen Dist 1.0m、72fps）

| f | 秒 | **`anchorZ`** | bboxH | 描画高 | boneRatio |
|---:|---:|---:|---:|---:|---:|
| 1075 | 35.83 | **0.7781** | 387 | 539 | 1.013 |
| 1081 | 36.03 | 0.7540 | 384 | 541 | 1.008 |
| 1084 | 36.13 | 0.7202 | 374 | **592** | **1.118** |
| 1090 | 36.33 | **0.7054** | 385 | 610 | 1.154 |
| 1123 | 37.43 | — | 386 | **663** | 1.176 |

**`anchorZ` が 0.5 秒で 0.778 → 0.705（−10%）に落ちる。**モデルはそれに従って手前へ寄り、
**描画が 541 → 663px（+23%）に増える。**

### bundle 自身の 2 つの信号が矛盾している

| | 36.0 → 36.3 秒 |
|---|---|
| `anchorZ`（深度）| 0.778 → 0.705（**−10%**）= 人が近づいた |
| `bboxH`（見かけの高さ）| 387 → 385（**変化なし**）= 人の見かけは同じ |

**人が 10% 近づいたなら bbox も 10% 大きくなるはず。**なっていない。
元動画（`docs/tmp/video_36s.png`）でも、この区間は**その場で膝を使ってボールを扱っているだけ**で移動していない。

**これは利用側（Unity）の問題ではなく、`anchor_z` が実距離を追えていない
（D-004、決定係数 human 0.143）ことの現れ。**

### 利用側でできること（要判断）

⑧ は `anchorZ` を信じて配置する。`anchorZ` が誤れば忠実であるほど誤る。

| | 案 | 代償 |
|---|---|---|
| 1 | **bbox と `anchorZ` が矛盾したら bbox を優先**（見かけの大きさが変わらないなら深度も変えない）| `anchorZ` が正しい場面で追従しなくなる |
| 2 | `anchorZ` の時間変化に上限を設ける | 本当に速く近づく場面で遅れる |
| 3 | bundle 側の D-004 の解決を待つ | いつになるか不明 |

**bundle 側へ報告する材料としてこの区間（f1075-1090）は良い実例。**
bbox が動かないのに `anchor_z` が 10% 動く、という**単一 shot 内で閉じた矛盾**なので、
生成側でも再現・検証しやすいはず。
