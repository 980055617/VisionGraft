...
## Screen Dist で Human だけ大きさが変わる（2026-09-05 実機報告 → 修正）

### 「変わらない」のが正しい

モデルは映像の対象に張り付いているので、スクリーンが遠ざかればモデルも遠ざかり、
**同じ画角を占めるのが正しい**。深度は追従している:

```csharp
zPlacement = screenDist - eps - popout;   // DecodeAnchorDepthMetersFromBundle
```

したがって **Else と動画本体が変わらないのは正常で、Human が変わるのがバグ**だった。

### 原因: スケールのロックが外れる契機に「視点の変更」が無かった

`GetOrLockModelLocalScale` は Human / Animal のスケールを shot 先頭で凍らせる。
外れる契機は **shot 境界とモデル変更の 2 つだけ**で、スクリーン距離の変更は入っていない。
深度だけ動いてスケールが据え置かれるので、人だけ大きさが変わって見えていた。

`OnRuntimeScreenDistanceSliderChanged` から `ReleaseLockedScalesForViewingChange()` を呼び、
`lockedModelLocalScaleByTrack` と `scaleRefinedByTrack` を捨てる。

**補正倍率（⑨ の `scaleRefineFactorByTrack`）は捨てない。** あれは track の姿勢に対する
比率で視点距離とは無関係。捨てるとその瞬間の姿勢で測り直され、モデル差し替えで踏んだのと
同じ跳ね（同一モデルへの差し替えで 15% 縮む）が起きる。

## モデルピッカーのプレビュー 3 件（2026-09-05 実機報告）

### 1. 大きいモデルが縮まない → clamp の下限が 1 だった

```
01_Wolf     boundsSize=(204.0, 107.0, 36.3)  scale=0.53  applied=1.00
03_Buffalo  boundsSize=(328.3, 196.0, 155.1) scale=0.33  applied=1.00
```

`Mathf.Clamp(scale, 1f, 3000f)` の**下限が 1**。prefab が目標より大きいモデルは
縮める必要があり倍率は 1 を下回るのに、切り上げられていた。
Buffalo はセルをはみ出して隣のセルと「表示しない」ボタンを覆っていた。

上限を 90 → 3000 に広げたとき（野球ボールが潰れる件）に**下限を見直していなかった**。
`Mathf.Clamp(scale, 0.0001f, 3000f)` にした。実測で 0.53 / 0.33 が反映され、
Buffalo がセル内に収まることをキャプチャで確認。

### 2. 動物の向きが揃わない → 前脚と後脚から向きを出して揃える

prefab の向きをそのまま使っていたので、Dog / Lion は右向き、Wolf / Buffalo は左向きだった。

`AnimalPoseApplier` と同じ規則（前脚の中点 − 後脚の中点）で forward を求め、
頭が右に来るようヨーを掛ける。ボーン名は `AnimalRigDefinition` の正規化済みの名前
（`front_l_upper` / `rear_l_upper` など）を直接引く。**リグのキャッシュは作らない**
（プレビューに必要なのは向きだけで、キャッシュ構築は重い）。

骨が見つからないモデル（球・機関車・人）は従来どおり。機関車の「奥行きが横幅より
長ければ 90 度回す」規則は、この判定が失敗したときだけ通るようにした。

### 3. ライオンのたてがみが透ける → **プレビュー固有。配置は正常**

同じモデルを実際に配置してキャプチャしたところ、たてがみは不透明で正しく描かれた。
マテリアルの不具合ではなく、プレビューの描画条件の問題。

**原因は未特定。** 有力なのは、たてがみが alpha clip 系のマテリアル
（同パッケージの `M_lynx_cards.mat` は `RenderType: TransparentCutout` / `_AlphaClip: 1` /
`_Cutoff: 0.35`）で、プレビューは lossy 0.03 程度まで縮むため、mipmap で
アルファの平均値が下がり cutoff を割る、という筋。ただし**確かめていない**。
拡大表示（ホバー）で改善するなら、この説明が正しいことになる。

## animal の配置精度: ⑧ が悪化させている（2026-09-05 実測）

D-006 で「sizeRatio が 0.592 → 1.176 と反転しただけ」と報告した件の切り分け。
旧ビルドは手元に無いので、**bundle の寄与ではなく ⑧ の寄与**を対照実験で測った。

`depthRefineMinRatio` を 1000 にすると ratio が下限を割り続けるので ⑧ が何もしなくなる。
これを対照にした（`-minRatio 1000`）。`bundle_animal.svb`（現行）全 2120 フレーム。

| | median | p10〜p90 | ±15% 外 | 誤差 median |
|---|---:|---|---:|---:|
| **A ⑧ 有効（既定 0.2）** | 1.211 | 1.005〜1.524 | **60.5%** | **21.1%** |
| **B ⑧ 無効** | 1.071 | 0.885〜1.440 | **47.9%** | **12.9%** |

**⑧ を切ったほうが良い。** 誤差の中央値が 21.1% → 12.9% に下がる。

track 別・見切れ別に見ても同じ向き:

| | A ⑧有効 | B ⑧無効 |
|---|---|---|
| track 0 | 1.185 / 58.8% 外 | 1.088 / 58.5% 外 |
| track 1 | 1.230 / 62.5% 外 | **1.012 / 35.5% 外** |
| 下端に接する | 1.253 / 60.2% 外 | **1.078 / 42.8% 外** |

**見切れフレームでも ⑧ を切ったほうが良い**のが意外な点。⑧ はまさに
「見切れで bbox に合わせるとモデルが縮む」を直すための仕組みなのに、逆に効いている。

### 疑い: 外挿の上限が旧ビルドで調整されている

`maxClippedHeightExtrapolation = 1.6` と `depthRefineMinRatio = 0.2` は
**2026-08-27 に bundle_animal で実測して決めた値**（Core.cs のコメント）。
しかしその時点の animal は shots が stale（15）で D-001 修正前の depth だった。
現行ビルドは shots 28・修正後 depth なので、**調整の前提が変わっている。**

⑧ 有効時の median が 1.21 と一貫して大きい側に出ているのは、
`ResolveUnclippedTargetHeight` が全高を過大に見積もっている形と整合する。

**残差**: ⑧ を切っても median 1.071 / ±15% 外 47.9% は残る。⑧ だけの問題ではない。

### 訂正: 上の A/B は評価基準が間違っていた（2026-09-05、同日）

`sizeRatio` の全区間中央値を **1.0 と比べていたのが誤り。**

`Core.cs` の `projectedBoneRatioTarget` のコメントに書いてある:

> `boneRatio` は姿勢で 1.0〜2.2 と動くため、基準フレーム（shot 先頭＝多くの場合は立位）で
> 1.0 に合わせても**全区間の中央値は 1.2 前後に残る**

スケールは **shot 先頭で 1.0 に合わせてロック**され、shot の途中は姿勢で動く。
つまり全区間の中央値が 1.2 前後なのは**設計どおり**で、欠陥ではない。

**shot 先頭だけで測り直した**（ロックが決まるフレーム。FK 後に測り直すので +2 フレームまで見る）。

| bundle_animal（shot 先頭 28 区間、n=83） | median | p10〜p90 | ±15% 外 |
|---|---:|---|---:|
| A 既定（⑧有効・extrap 1.6） | 1.169 | 1.032〜1.544 | 63.9% |
| B ⑧ 無効 | 1.161 | 0.996〜1.252 | 53.0% |
| C ⑧有効・見切れ補正なし | **1.143** | 1.002〜1.248 | **48.2%** |
| E ⑧有効・extrap 1.3 | 1.169 | 1.032〜1.421 | 63.9% |

| bundle_human（Person、基準フレーム n=3） | median | ±15% 外 |
|---|---:|---:|
| **既定（見切れ補正あり）** | **0.981** | **0.0%** |
| 見切れ補正なし | 1.107 | 33.3% |

### 分かったこと

**1. human は基準フレームで 0.981 と正しく合っている。** 設計どおり動いている。
`extendTargetHeightForClippedBBox` を切ると 1.107 に**悪化**するので、既定は変えない。
（human は 1 shot しかないので n=3。統計的な重みは小さいが、方向は明確）

**2. animal は基準フレームでも 1.14〜1.17 と 15% ほど大きい。**
⑧ の有無や見切れ補正の設定を振っても 1.14〜1.17 の範囲でしか動かない。
**主因はそのどれでもない。** C（見切れ補正なし）がわずかに良いが、
human を悪化させるので既定の変更には値しない。

**3. 「精度が上がっていない」という前回の判断は誤り。**
旧ビルドの 0.592（端に接しないフレーム）は期待値 1.2 前後から大きく外れており、
現行の 1.21（全区間）/ 1.16（基準フレーム）は期待値に近い。**大きく改善している。**
比較対象を 1.0 に取っていたために「反転しただけ」と読んでしまった。

### 残っている問題

animal だけ基準フレームで 15% 大きい理由は未特定。human は 0.98 なので、
**Human と Animal で骨格の投影高の測り方が食い違っている**筋が濃厚
